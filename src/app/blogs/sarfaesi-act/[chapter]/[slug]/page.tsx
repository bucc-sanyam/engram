...
export function generateStaticParams() {
  return SARFAESI_CHAPTERS.flatMap((c) =>
...
export async function generateMetadata({
  ...
  const hit = getSarfaesiSection(chapterSlug, slug);
  if (!hit) return { title: `Not found · ${SARFAESI_SERIES_TITLE}` };
  return {
    title: `${hit.section.title} · ${hit.chapter.title} · ${SARFAESI_SERIES_TITLE} · Knovis`,
    description: hit.section.summary,
  };
}

/** A single section blog in The SARFAESI Playbook. */
export default async function SarfaesiSectionPage({
  params,
}: {
  params: Promise<{ chapter: string; slug: string }>;
}) {
  ...
  const { prev, next } = sarfaesiNeighbors(chapter.slug, section.slug);
  const number = sarfaesiSectionNumber(chapter.slug, section.slug);
          ...
          <nav className="mt-12 grid gap-3 sm:grid-cols-2">
            ...
            {next ? (
              ...
              >
                ...
                <span className="mt-1 block font-medium text-[#a78bfa]">
                  Back to {SARFAESI_SERIES_TITLE}
                </span>
              </Link>
            )}
          </nav>
